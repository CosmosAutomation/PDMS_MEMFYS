...
void controlAlarm() {
  ...
  bool anyLoss = false;
  for (int i = 0; i < 5; i++) {
    if (storedIDs[i] && millis() - lastReceivedTimes[i] >= 30000) {// if any device does not communicate for 30 seconds
      anyLoss = true;
      uint8_t ledIndex = i * 3;  // Calculate LED index based on array position
      sr.setNoUpdate(ledIndex, LOW);
      sr.setNoUpdate(ledIndex + 1, LOW); // set PH-F, PH-I & PH-HV leds low
      sr.setNoUpdate(ledIndex + 2, LOW);
      sr.setNoUpdate(i + 15, LOW);  //set comm LED low to show there is no communication
      sr.updateRegisters();
      //break;
    }
  }
